...
export type AuthState = {
    ...
    logout: () => Promise<void>,
    signup: (firstName: string, lastName: string, phone: string, location: string, pin: string) => Promise<void>,
}
...
const init: AuthState = {
    ...
    error: null,
    clearError: () => {},
    login: () => {return new Promise<void>(() => {})},
    logout: () => {return new Promise<void>(() => {})},
    signup: () => {return new Promise<void>(() => {})},
}
...
export const AuthProvider: React.FC<Props> = ({ children }) => {
    ...
    const setFirebaseError = (e: FirebaseError): void => {
        ...
        if (e.code === "auth/wrong-password") setError(new Error("PIN is incorrect"));
          ...
          setError(new Error("Access to this account temporarily disabled due to too many login attempts. Please try again later or reset password"))
    ...
    const signup = async (firstName: string, lastName: string, phone: string, location: string, pin: string): Promise<void> => {
        try {
            ...
            if (e instanceof FirebaseError) {
                ...
                setError(e as Error); //jen check this
            }
            setUser(null);
        }
    }
